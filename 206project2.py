...
def grab_headlines():
    ...
    div=soup.find_all("div",class_="view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266")
    for tag in div:
        a=tag.find_all("li")
    listofheadlines=[]
    for b in a:
...
def get_umsi_data():
    ...
    for tag in h2:
        ...
        if len(b)==3:
            if b[1][2]==">":
                people.append(b[1][3:])
    div= soup.find_all("div", class_="field-items")
    for tag in div:
        strtag=str(tag)
        splittag=strtag.split(">")
        if len(splittag)==5:
            titles.append(splittag[-3][:-5])
    ...
    urls=[]
    for a in numbers[1:]:
        url="https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page=" + str(a)     
        urls.append(url)
    for a in urls:
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ...
        for tag in div:
            ...
            if len(splittag)==5:
                titles.append(splittag[-3][:-5])
    for a,b in zip(people,titles):
        umsi_titles[a] = b
    for a in umsi_titles:
        if umsi_titles[a]=="Associate Professor of Music, School of Music, Theatre &amp; Dance and Associate Professor of Information, School of Information":
            umsi_titles[a]="Associate Professor of Music, School of Music, Theatre & Dance and Associate Professor of Information, School of Information"
    #for some reason it was chaning the & symbol to "&amp;" so i hard coded it to not do that.
    #all the other code was working perfectly besides that one small thing
    return (umsi_titles)
